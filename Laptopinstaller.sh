...
sudo apt-get install postgres postgres-contrib -y
...
sudo ufw status
tree /media/$USER/Webdevdata
sudo cp -R /media/$USER/Webupdatedb/nginx/conf.d/ -t /etc/nginx/
sudo cp -R /media/$USER/Webupdatedb/supervisor/conf.d/ -t /etc/supervisor/
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
   # Web Cert installation for secure your web domain 
echo "Certbot Secure application" 
sudo add-apt-repository ppa:certbot/certbot
sudo apt install certbot python3-certbot-nginx -y
sudo cp -R /media/$USER/C8B063EAB063DD86/Webupdatedb/idatabots -t /var/www/ # Get the directory of the working file
